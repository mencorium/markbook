...
def _info(d: AttendanceDay) -> AttendanceDayInfo:
    return AttendanceDayInfo(d.id, d.class_id, d.date, [e.student_id for e in d.entries], [e.student_id for e in d.entries if not e.present])


def list_days(class_id: int | None = None) -> list[AttendanceDayInfo]:
...
def save_day(class_id: int, date: dt.date, roster: list[int], absent: set[int]) -> AttendanceDayInfo:
    ...
    with session_scope() as s:
        d = s.scalar(select(AttendanceDay).where(AttendanceDay.class_id == class_id, AttendanceDay.date == date))
        if d is None:
            d = AttendanceDay(class_id=class_id, date=date)
            s.add(d)
            s.flush()
